/**
   ...
   * Follow https://github.com/AlyaaXd
*/
...
module.exports = hisoka = async (hisoka, m, chatUpdate, store) => {
	...
	const isBotAdmins = m.isGroup ? groupAdmins.includes(hisoka.user.id) : false
        ...
        switch(command) {
	    case 'sc': {
                m.reply('Script : https://github.com/AlyaaXzy/AlyaaXzy-Md\n\n Dont Forget Give Star\n\nDonate : 085730903853\n\n Dont Forget Donate')
            }
            break
            case 'chat': {
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
            ...
            case 'owner': case 'creator': {
                let vcard = 'BEGIN:VCARD\n' // metadata of the contact card
                    ...
                    + 'TEL;type=CELL;type=VOICE;waid=6289505165400:6289505165400\n' // WhatsApp ID + phone number
                    + 'END:VCARD'
                hisoka.sendMessage(m.chat, { contacts: { displayName: 'AlyaaXzy-Md.', contacts: [{ vcard }] } }, { quoted: m })
            }
            break
            case 'eval': {
